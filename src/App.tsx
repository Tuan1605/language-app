    ...
    <div className="min-h-screen flex bg-white text-[#4b4b4b] selection:bg-[#ddf4ff] selection:text-[#1cb0f6]">
      ...
      <aside className="hidden lg:flex flex-col w-64 border-r-2 border-[#e5e5e5] p-4 fixed h-full bg-white z-50">
        ...
        <div className="mt-auto p-4 bg-[#f7f7f7] rounded-2xl border-2 border-[#e5e5e5]">
           <p className="text-[9px] font-black text-[#afafaf] uppercase tracking-widest mb-2 text-center">Daily Goal</p>
           <div className="lingo-progress mb-2"><div className="lingo-progress-inner bg-[#58cc02]" style={{ width: '65%' }}></div></div>
           <p className="text-[10px] font-black text-center">65% COMPLETED</p>
        </div>
      </aside>
      ...
      <main className="flex-1 min-h-screen lg:pl-64">
        ...
        <div className="max-w-[1024px] mx-auto w-full flex flex-col items-center">
          
          {/* Content Wrapper with consistent padding */}
          <div className="w-full p-4 sm:p-8 md:p-12 pb-24 lg:pb-12 flex flex-col">
            
            {mode === 'home' && (
              <div className="space-y-12 animate-in fade-in duration-700 w-full flex flex-col items-center">
                {/* Top Bar - Stays wide within the container */}
                <div className="flex items-center justify-between pb-6 border-b-2 border-[#e5e5e5] w-full max-w-2xl">
                  <div className="flex items-center gap-4 sm:gap-8">
                    <div className="flex items-center gap-2"><span className="text-xl">🔥</span><span className="font-black text-[#ff9600] text-base leading-none">3</span></div>
                    <div className="flex items-center gap-2"><span className="text-xl">💎</span><span className="font-black text-[#1cb0f6] text-base leading-none">120</span></div>
                    <div className="flex items-center gap-2"><span className="text-xl">❤️</span><span className="font-black text-[#ff4b4b] text-base leading-none">5</span></div>
                  </div>
                  <div className="w-9 h-9 rounded-full border-2 border-[#e5e5e5] bg-[#f7f7f7] flex items-center justify-center text-lg shadow-sm">👤</div>
                </div>

                {/* Learning Path - Perfectly Centered */}
                <div className="flex flex-col items-center gap-12 w-full pt-4">
                  <div className="w-full max-w-sm bg-[#58cc02] rounded-2xl p-6 text-white shadow-[0_6px_0_0_#46a302] mb-4">
                    ...
                    <p className="font-bold text-white/90 text-sm mt-1 opacity-80 text-center sm:text-left">Level: Intermediate</p>
                  </div>
                  ...
                  <div className="flex flex-col items-center gap-12 w-full relative">
                    ...
                    <div className="z-10 sm:-ml-24 flex flex-col items-center">
                      ...
                      <span className="mt-2 text-[10px] font-black text-[#afafaf] uppercase tracking-widest text-center">TOEIC</span>
                    </div>
                    ...
                    <div className="z-10 sm:-mr-24 flex flex-col items-center">
                      ...
                      <span className="mt-2 text-[10px] font-black text-[#afafaf] uppercase tracking-widest text-center">JLPT N2</span>
                    </div>
                    ...
                    <div className="z-10 flex flex-col items-center">
                      ...
                      <span className="mt-2 text-[10px] font-black text-[#afafaf] uppercase tracking-widest text-center">AUDIO</span>
                    </div>
            ...
            {/* Action Screens - Perfectly Centered within their max-width */}
                ...
                {mode === 'review' && (
                  <div className="space-y-10 w-full">
                    ...
                    {reviewQueue.length > 0 ? <FlashcardView card={reviewQueue[currentCardIndex]} onRate={handleRateCard} /> : (
                      <div className="lingo-card text-center space-y-8 py-12 w-full">
                  ...
                  <div className="space-y-8 w-full">
                    {!selectedLesson ? (
                      <div className="space-y-6 w-full">
                        <h2 className="text-3xl font-black uppercase tracking-tight text-center sm:text-left">Audio Library</h2>
                        <div className="grid grid-cols-1 gap-4 w-full">
                          {MOCK_LISTENING_LESSONS.map((lesson) => (
                            <div key={lesson.id} onClick={() => setSelectedLesson(lesson)} className="lingo-card flex items-center gap-6 p-6 cursor-pointer hover:scale-[1.01] active:scale-[0.98] transition-all">
                              ...
                              <div className="flex-1"><h6 className="font-black text-xl leading-tight">{lesson.title}</h6><p className="text-[10px] font-black text-[#afafaf] uppercase tracking-widest mt-1 text-left">Tap to start lesson</p></div>
                            </div>
                          ))}
                        </div>
                      </div>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 h-20 bg-white border-t-2 border-[#e5e5e5] flex items-center justify-around px-4 z-50">
        <button onClick={() => setMode('home')} className={`flex flex-col items-center gap-1 ${mode === 'home' ? 'text-[#1cb0f6]' : 'text-[#afafaf]'}`}>
          ...
          <span className="text-[10px] font-black tracking-widest">LEARN</span>
        </button>
        <button onClick={() => setMode('review')} className={`flex flex-col items-center gap-1 ${mode === 'review' ? 'text-[#1cb0f6]' : 'text-[#afafaf]'}`}>
          ...
          <span className="text-[10px] font-black tracking-widest">CARDS</span>
        </button>
        <button onClick={() => setMode('quiz')} className={`flex flex-col items-center gap-1 ${mode === 'quiz' ? 'text-[#1cb0f6]' : 'text-[#afafaf]'}`}>
          ...
          <span className="text-[10px] font-black tracking-widest">QUESTS</span>
        </button>
        <button onClick={() => setMode('listening')} className={`flex flex-col items-center gap-1 ${mode === 'listening' ? 'text-[#1cb0f6]' : 'text-[#afafaf]'}`}>
          ...
          <span className="text-[10px] font-black tracking-widest">AUDIO</span>
        </button>
      </nav>
    </div>
  );
