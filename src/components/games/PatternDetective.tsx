...
export function PatternDetective({ onComplete }: { onComplete: () => void; }) {
  ...
  const currentProgress = gameProgress['grammar-detective'] || { highScore: 0, maxLevel: 1 };
  ...
  const nextQuestion = () => {
    if (currentIndex + 1 < levelPoints.length) {
      ...
      if (currentLevel < totalLevels) {
        updateGameProgress('grammar-detective', score, currentLevel + 1);
  ...
  const finishGame = (finalScore: number) => { setGameState('gameover'); if (finalScore > 0) { addExp(finalScore); updateGameProgress('grammar-detective', finalScore, currentLevel); } };

  if (gameState === 'loading') return <GameLoading text="Loading grammar..." />;
  
  if (gameState === 'ready') return (
    <GameShell title="Pattern Detective" icon={<Search className="w-5 h-5" />} onBack={onComplete}>
      <div className="flex-1 flex flex-col items-center justify-center p-4">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center w-full">
          <h2 className="text-2xl font-black text-text-main mb-2">Pattern Detective</h2>
          <p className="text-sm font-bold text-text-muted mb-6">Nhận diện cấu trúc ngữ pháp từ câu ví dụ</p>
  ...
  if (gameState === 'gameover') return (<GameShell title="Pattern Detective" icon={<Search className="w-5 h-5" />} onBack={onComplete}><GameOverScreen score={score} expEarned={score} highScore={currentProgress.highScore} isWin={score > 0} onRestart={() => startGame(currentLevel)} onMenu={onComplete} /></GameShell>);
  if (gameState === 'level_complete') return (<GameShell title="Pattern Detective" icon={<Search className="w-5 h-5" />} onBack={onComplete}><LevelCompleteScreen currentLevel={currentLevel} totalLevels={totalLevels} correct={levelCorrect} total={levelPoints.length} score={score} onNextLevel={nextLevel} /></GameShell>);
