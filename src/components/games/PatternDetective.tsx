...
import { Trophy, Timer, Zap, Search } from 'lucide-react';
...
export function PatternDetective({ onComplete, difficulty = 'medium' }: { onComplete: () => void; difficulty?: 'easy' | 'medium' | 'hard' }) {
  ...
  const finishGame = (finalScore: number) => {
    ...
    if (finalScore > 0) {
      addExp(finalScore);
      setGameHighScore('grammar-detective', difficulty, finalScore);
  ...
  if (gameState === 'gameover') {
    return (
      <div className="flex-1 flex flex-col items-center justify-center">
        ...
        {gameHighScores['grammar-detective']?.[difficulty] > 0 && (
          <p className="text-sm font-bold text-gold mb-8">Best: {gameHighScores['grammar-detective'][difficulty]}</p>
        )}
      ...
      <div className="flex-1 flex flex-col items-center justify-center max-w-2xl mx-auto w-full">
        <motion.div key={currentIndex} initial={{ x: 50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="w-full">
          <div className="flex items-center justify-center gap-2 mb-3">
            ...
            <p className="text-xs font-bold text-text-muted uppercase tracking-wider">Grammar pattern nào được dùng trong câu sau?</p>
          </div>

          <div className="bg-gray-bg rounded-2xl p-6 border-2 border-gray-path text-center mb-4">
            <p className="text-xl md:text-2xl font-bold text-text-main leading-relaxed">
              "{current?.example}"
            </p>
            {current?.exampleTranslation && (
              <p className="text-sm text-text-muted mt-2">{current.exampleTranslation}</p>
            )}
          </div>

          <p className="text-sm font-bold text-text-muted text-center mb-4">{current?.meaning}</p>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {options.map((opt, idx) => {
              const isSelected = selectedAnswer === opt;
              const isCorrectOpt = opt === current?.pattern;
              let btnClass = 'bg-white border-gray-path hover:border-purple hover:bg-purple/5';
